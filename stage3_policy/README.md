# stage3_policy — Policy / task reasoning (owner: Bidipta)
...
- **Geometry** comes from `assets/bimanual_scene.xml` and the arm XMLs. `tests/test_config.py` re-derives it from the XML, so if Azeem moves the drawer or table the test fails instead of the planner silently using old numbers.
...
- **Minimal venv:** `RESULT: FAIL`, exit 1. Stage 2's `perceive(None)` returns table-corner coordinates (z ≈ 0.02), and Stage 3 refuses them as a frame mismatch. This is the honest state; merging it means `main` no longer "passes" the stub pipeline, **a decision for Alex**.
...
**Pipeline correction** (`common/pipeline.py`, owned by Alex; a separate, backward-compatible change):
...
- **Export:** ACT (ResNet-18 + transformer) is a more realistic OpenVINO export target for Lauren.
...
### Proposed demonstration schema (for Muhammad — `learned/schema.py`, not yet agreed)
...
Proposed contract changes: [`CONTRACT_PROPOSAL.md`](CONTRACT_PROPOSAL.md) (**not approved**).
