...
**Status:** draft from Bidipta (Stage 3) for discussion. None of this is in
`common/types.py` or `CONTRACTS.md`. **No stage should rely on these fields until
a PR updates both files and the owners below agree.** Until then Stage 3 supports
...
- **Produce:** Stage 2 `perceive` (Lauren/Abdullah), Stage 4 `ExecutionResult.final_scene` (Azeem).
- **Consume:** Stage 3 `plan` (Bidipta), Stage 6 `verify` (Abdullah), `common/pipeline.py` logs (Alex).
...
- **Produce:** Stage 3. **Consume:** Stage 4 `execute` (Azeem), Stage 6 (Abdullah) for diagnostics.
...
- **Owner:** Alex (`common/pipeline.py`), with Azeem.
...
### P4. Stage 1 task semantics (Alex)
...
### P5. Executor motor-policy hook (Azeem ↔ Bidipta, needed before any learned policy runs)
